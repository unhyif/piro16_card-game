...
class Game(models.Model): # ModelForm 만들 때 field 수동으로 설정 (not "__all__")
    ...
    attacker = models.ForeignKey(UserList.user, on_delete=models.CASCADE, related_name="attack", verbose_name="공격자") # "공격하기" 누르면 views.py에서 request.user로 설정(form으로 선택받지 않음)
    # related_name: ex) attacker가 user인 Games
    defender = models.ForeignKey(UserList.user, on_delete=models.CASCADE, related_name="defend", verbose_name="수비자", blank=True) # forms.py에서 label="공격할 상대는?", field 값 목록에서 attacker exclude 하기
    ...
    winner = models.ForeignKey(UserList.user, on_delete=models.CASCADE, verbose_name="승자", blank=True, null=True) # "반격하기" 누르면 views.py에서 rule과 각자의 num에 따라 field 값 채워줌
